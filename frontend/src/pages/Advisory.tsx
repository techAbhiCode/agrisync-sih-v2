  ...
  return (
    <div className="h-full flex flex-col items-center p-4 sm:p-6 lg:p-8 overflow-hidden">
      ...
      <div className="w-full max-w-4xl mb-6">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-zinc-900/60 p-4 rounded-3xl backdrop-blur-xl border border-zinc-800">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-gradient-to-br from-lime-500/20 to-emerald-500/20 rounded-xl border border-lime-500/30">
              <Sparkles className="h-6 w-6 text-lime-400" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-white tracking-tight">AI Services</h1>
              <p className="text-zinc-400 text-sm">Powered by AgriGenius</p>
            </div>
          </div>
          ...
          <div className="flex bg-zinc-950/50 p-1 rounded-2xl border border-zinc-800">
            <button
              ...
              className={`px-6 py-2 rounded-xl text-sm font-medium transition-all ${
                activeTab === 'chat' 
                  ? 'bg-lime-500 text-zinc-950 shadow-lg' 
      ...
      <div className="w-full max-w-4xl flex-1 flex flex-col bg-zinc-900/40 backdrop-blur-xl border border-zinc-800/50 rounded-3xl overflow-hidden shadow-2xl relative min-h-[500px]">
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'chat' && (
            ...
            >
                    ...
                    <div className={`flex gap-3 max-w-[85%] ${msg.sender === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                      ...
                      <div className="flex-shrink-0">
                        {msg.sender === 'ai' ? (
                          ...
                          <div className="w-8 h-8 rounded-full bg-lime-500/20 border border-lime-500/30 flex items-center justify-center shadow-lg">
                      ...
                      <Card className={`p-4 border-none shadow-lg ${
                        msg.sender === 'user' 
                        ? 'bg-gradient-to-br from-lime-500 to-emerald-600 text-white rounded-2xl rounded-tr-sm' 
                        : 'bg-zinc-800/80 text-zinc-200 rounded-2xl rounded-tl-sm backdrop-blur-md border border-zinc-700/50'
                      }`}>
                        {msg.sender === 'user' ? (
                ...
                {isChatLoading && (
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      <Card className="p-4 bg-zinc-800/80 border-none rounded-2xl rounded-tl-sm flex items-center gap-3">
              ...
              {/* Input Area */}
              <div className="p-4 sm:p-6 bg-zinc-950/80 border-t border-zinc-800/80 backdrop-blur-xl">
                <form onSubmit={handleChatSubmit} className="relative flex items-center">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-14 py-4 bg-zinc-900 border border-zinc-800 focus:border-lime-500/50 focus:ring-1 focus:ring-lime-500/50 rounded-2xl text-zinc-100 placeholder-zinc-500 transition-all shadow-inner"
                  />
                  <button
                    type="submit"
                    disabled={!input.trim() || isChatLoading}
                    className="absolute right-2 p-2.5 bg-lime-500 hover:bg-lime-400 disabled:bg-zinc-800 disabled:text-zinc-500 text-zinc-950 rounded-xl transition-colors shadow-lg group"
                  >
          ...
          {activeTab === 'recommendation' && (
            ...
            >
              <div className="bg-gradient-to-br from-emerald-900/30 to-zinc-900 border border-emerald-500/20 p-6 rounded-3xl shadow-xl">
                <h2 className="text-2xl font-bold text-white mb-2 flex items-center gap-2">
                  <Target className="h-6 w-6 text-emerald-400" />
                  Smart Crop Recommendations
                </h2>
                <p className="text-zinc-400 mb-6">Enter your location to get AI-analyzed crop suggestions based on live weather and regional climate data.</p>
                ...
                <form onSubmit={handleRecSubmit} className="relative flex items-center max-w-xl">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-14 py-4 bg-zinc-950 border border-zinc-700 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 rounded-2xl text-zinc-100 placeholder-zinc-500 transition-all shadow-inner"
                  />
                  <button
                    type="submit"
                    disabled={!locationInput.trim() || isRecLoading}
                    className="absolute right-2 p-2.5 bg-emerald-500 hover:bg-emerald-400 disabled:bg-zinc-800 disabled:text-zinc-500 text-zinc-950 rounded-xl transition-colors shadow-lg group flex items-center justify-center min-w-[44px]"
                  >
              ...
              {recData && (
                ...
                >
                  {/* Weather Info */}
                  <div className="flex items-center justify-between bg-zinc-800/50 p-5 rounded-2xl border border-zinc-700/50">
                    <div className="flex items-center gap-4">
                      <div className="p-3 bg-blue-500/20 rounded-xl">
                        <CloudSun className="h-6 w-6 text-blue-400" />
                      </div>
                      <div>
                        <h3 className="text-white font-medium text-lg">Live Weather in {recData.location}</h3>
                        {recData.weather ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {recData.recommendations.map((crop, idx) => (
                      ...
                      >
                        <Card className="p-5 bg-zinc-800/80 border-zinc-700/50 h-full flex flex-col hover:border-emerald-500/50 transition-colors">
                          <div className="text-4xl mb-3">{crop.icon}</div>
                          <h4 className="text-lg font-bold text-emerald-400 mb-2">{crop.name}</h4>
                          <p className="text-sm text-zinc-300 flex-1 leading-relaxed">{crop.reason}</p>
                        </Card>
                      </motion.div>
                    ))}
                  </div>
                  ...
                  {/* Expert Advice */}
                  <Card className="p-6 bg-gradient-to-r from-zinc-900 to-zinc-800/80 border-l-4 border-l-emerald-500 border-t-zinc-700/50 border-r-zinc-700/50 border-b-zinc-700/50">
                    <div className="flex gap-4">
                      <div className="mt-1">
                        <Info className="h-6 w-6 text-emerald-500" />
                      </div>
                      <div>
                        <h4 className="text-white font-bold mb-2">Expert Advice</h4>
